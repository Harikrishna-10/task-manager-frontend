...
const ViewTaskDetails = () => {
  ...
  const updateTodoChecklist = async (index) => {
    // FIX 2: Use 'todoChecklist' (lowercase 'l') to match Schema
    ...
    if (checklist && checklist[index]) {
      ...
      try {
        ...
        if (response.status === 200) {
          setTask(response.data?.task || { ...task, todoChecklist: checklist });
        }
      } catch (error) {
        console.error("Error updating checklist", error);
        checklist[index].completed = !checklist[index].completed;
        setTask({ ...task, todoChecklist: checklist });
      }
    }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 mt-4">
            <div className="form-card col-span-3">
              <div className="flex items-center justify-between">
                <h2 className="text-sm md:text-xl md:text-xl font-medium">
                  {task?.title}
                </h2>
                <div
...
const Attachement = ({ link, index, onClick }) => {
  return (
    <div
